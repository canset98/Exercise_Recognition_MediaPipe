...
class FullBodyPoseEmbedder(object):
    ...
    def _get_pose_distance_embedding(self, landmarks):
        embedding = np.array([
            ...
            self._get_distance(self._get_average_by_names(landmarks, 'left_wrist', 'left_ankle'),
                               landmarks[self._landmark_names.index('left_hip')])[0],
            self._get_distance(self._get_average_by_names(landmarks, 'right_wrist', 'right_ankle'),
...
# Video capture setup for webcam
...
while cap.isOpened():
    ...
    if results.pose_landmarks:
        ...
        if pose_landmarks.shape == (33, 3):  # Ensure correct landmarks shape
            ...
            statuses['pushups'], counters['pushups'] = update_counter(most_common_label, statuses['pushups'],
                                                                      counters['pushups'], "pushups_up", "pushups_down")
            statuses['jumping_jacks'], counters['jumping_jacks'] = update_counter(most_common_label,
                                                                                  statuses['jumping_jacks'],
                                                                                  counters['jumping_jacks'],
                                                                                  "jumping_jacks_up",
                                                                                  "jumping_jacks_down")
            statuses['pullups'], counters['pullups'] = update_counter(most_common_label, statuses['pullups'],
                                                                      counters['pullups'], "pullups_up", "pullups_down")
            statuses['situps'], counters['situps'] = update_counter(most_common_label, statuses['situps'],
                                                                    counters['situps'], "situp_up", "situp_down")
            statuses['squats'], counters['squats'] = update_counter(most_common_label, statuses['squats'],
                                                                    counters['squats'], "squats_up", "squats_down")

            # Display the counter for the current exercise only
            if most_common_label in ["pushups_up", "pushups_down"]:
                cv2.putText(frame, f"Push-ups: {counters['pushups']}", (frame_width - 500, 100),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            elif most_common_label in ["jumping_jacks_up", "jumping_jacks_down"]:
                cv2.putText(frame, f"Jumping Jacks: {counters['jumping_jacks']}", (frame_width - 500, 100),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            elif most_common_label in ["pullups_up", "pullups_down"]:
                cv2.putText(frame, f"Pull-ups: {counters['pullups']}", (frame_width - 500, 100),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            elif most_common_label in ["situp_up", "situp_down"]:
                cv2.putText(frame, f"Sit-ups: {counters['situps']}", (frame_width - 500, 100),
                            cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2, cv2.LINE_AA)
            elif most_common_label in ["squats_up", "squats_down"]:
                cv2.putText(frame, f"Squats: {counters['squats']}", (frame_width - 500, 100), cv2.FONT_HERSHEY_SIMPLEX,
